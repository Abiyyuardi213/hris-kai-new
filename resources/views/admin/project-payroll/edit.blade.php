@extends('layouts.app')
@section('title', 'Edit Payroll Project')
    ...
    <div class="flex flex-col space-y-6 max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight text-zinc-900">Edit Payroll Project</h2>
                    <p class="text-zinc-500 text-sm mt-1">Ubah data pembayaran project untuk pegawai terpilih.</p>
                </div>
            </div>
        </div>

        <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-zinc-200/50 overflow-hidden">
            <form action="{{ route('admin.project-payroll.update', $projectPayroll->id) }}" method="POST" class="p-8 space-y-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2.5">
                        ...
                        <select name="pegawai_id" required id="pegawai_id"
                            ...
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}" {{ old('pegawai_id', $projectPayroll->pegawai_id) == $employee->id ? 'selected' : '' }}>
                    ...
                    <div class="space-y-2.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="project_name" value="{{ old('project_name', $projectPayroll->project_name) }}" required
                    ...
                    <div class="space-y-2.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="total_pay" value="{{ old('total_pay', $projectPayroll->total_pay) }}" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2.5">
                            ...
                            <select name="month" required
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('month', $projectPayroll->month) == $i ? 'selected' : '' }}>
                        ...
                        <div class="space-y-2.5">
                            ...
                            <select name="year" required
                                ...
                                @for ($i = date('Y'); $i >= 2020; $i--)
                                    <option value="{{ $i }}"
                                        {{ old('year', $projectPayroll->year) == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
                ...
                <div class="space-y-2.5">
                    ...
                    <textarea name="keterangan" rows="4" placeholder="Jelaskan alasan pemberian payroll tambahan berdasarkan kinerja..."
                        class="w-full rounded-xl border border-zinc-200 bg-white/50 p-4 text-sm focus:ring-2 focus:ring-zinc-900 outline-none transition-all focus:bg-white resize-none">{{ old('keterangan', $projectPayroll->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-[10px] text-red-500 font-bold ml-1 tracking-wide">{{ $message }}</p>
                    @enderror
                </div>
