...
@section('content')
    <div class="max-w-4xl mx-auto flex flex-col space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Edit Pengumuman</h2>
                <p class="text-zinc-500 text-sm">Sesuaikan informasi pengumuman yang sudah ada.</p>
            </div>
        </div>
                ...
                <div class="bg-white rounded-3xl border border-zinc-200 shadow-sm p-8 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="content" id="editor" rows="15" required
                            class="w-full px-5 py-4 rounded-2xl border border-zinc-200 text-sm focus:ring-2 focus:ring-zinc-900 transition-all">{{ old('content', $announcement->content) }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <select name="category" required
                                ...
                                <option value="Umum" {{ $announcement->category == 'Umum' ? 'selected' : '' }}>Umum
                                </option>
                                <option value="Penting" {{ $announcement->category == 'Penting' ? 'selected' : '' }}>Penting
                                    (Urgent)</option>
                                <option value="HR" {{ $announcement->category == 'HR' ? 'selected' : '' }}>HR &
                                    Kebijakan</option>
                                <option value="Acara" {{ $announcement->category == 'Acara' ? 'selected' : '' }}>Acara /
                                    Event</option>
                            </select>
                        </div>
                        ...
                        <div class="pt-4 border-t border-zinc-50">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span class="text-sm font-bold text-zinc-900 select-none">Aktif Sekarang</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6 space-y-6">
                    <div class="space-y-4">
                        @if ($announcement->image)
                            <div class="relative group h-40 w-full rounded-2xl overflow-hidden border border-zinc-200">
                                ...
                                <div
                                    ...
                                    <span class="text-[10px] text-white font-bold uppercase tracking-widest">Ganti
                                        Gambar</span>
                                </div>
                            </div>
                        @endif
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Gambar
                                Cover</label>
                        ...
                        @if ($announcement->file_attachment)
                            <div class="p-3 bg-zinc-50 rounded-xl flex items-center gap-3 border border-zinc-100">
                                <i data-lucide="file" class="h-4 w-4 text-zinc-400"></i>
                                <span
                                    class="text-[10px] font-bold text-zinc-600 truncate flex-1">{{ basename($announcement->file_attachment) }}</span>
                            </div>
                        @endif
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Ganti File
                                Lampiran</label>
