...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight">Manajemen Lembur</h2>
            <button onclick="openModal('addOvertimeModal')"
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-zinc-200">
            <form action="{{ route('admin.overtime.index') }}" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="type"
                            ...
                            <option value="request" {{ request('type') == 'request' ? 'selected' : '' }}>Pengajuan (Pegawai)
                            </option>
                            <option value="assignment" {{ request('type') == 'assignment' ? 'selected' : '' }}>Penugasan
                                (Admin)</option>
                        </select>
                    </div>
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @forelse ($overtimes as $overtime)
                            <tr class="group hover:bg-zinc-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-zinc-900 capitalize">{{ $overtime->pegawai->nama_lengkap }}
                                    </div>
                                    <div class="text-[10px] text-zinc-500 font-medium">NIP: {{ $overtime->pegawai->nip }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @if ($overtime->type === 'assignment')
                                        <span
                                            class="inline-flex items-center rounded-md bg-blue-50 px-2 py-0.5 text-[10px] font-bold uppercase text-blue-700 ring-1 ring-inset ring-blue-700/10">
                                            Penugasan
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center rounded-md bg-zinc-100 px-2 py-0.5 text-[10px] font-bold uppercase text-zinc-600">
                                            Pengajuan
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-xs text-zinc-600 line-clamp-2 max-w-[200px]">{{ $overtime->reason }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusColors = [
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold uppercase ring-1 ring-inset {{ $statusColors[$overtime->status] }}">
                                        {{ $overtime->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        @if ($overtime->status === 'pending')
                                            <button onclick="openApprovalModal('{{ $overtime->id }}')"
                                        ...
                                        @endif
                                        <button onclick="confirmDelete('{{ $overtime->id }}')"
                                ...
                                <td colspan="6" class="px-6 py-16 text-center text-zinc-500">
                                    <div class="flex flex-col items-center justify-center space-y-3">
                                        ...
                                        <div class="text-center">
                                            <p class="font-medium text-zinc-900">Belum ada data lembur</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
    ...
    <!-- Modal Add Overtime (Assignment) -->
    <div id="addOvertimeModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-zinc-900/75 transition-opacity backdrop-blur-sm"
            onclick="closeModal('addOvertimeModal')"></div>
                    ...
                    <form action="{{ route('admin.overtime.store') }}" method="POST">
                        ...
                        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <div class="flex items-center justify-between mb-6">
                                <h3 class="text-lg font-bold text-zinc-900">Buat Penugasan Lembur</h3>
                                <button type="button" onclick="closeModal('addOvertimeModal')"
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-bold text-zinc-900 mb-1">Tanggal Lembur</label>
                                    <input type="date" name="date" required
                                        class="block w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-zinc-900 outline-none transition-all">
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-bold text-zinc-900 mb-1">Jam Mulai</label>
                                        <input type="time" name="start_time" required
                                            class="block w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-zinc-900 outline-none transition-all">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-bold text-zinc-900 mb-1">Jam Selesai</label>
                                        <input type="time" name="end_time" required
                                            class="block w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-zinc-900 outline-none transition-all">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-zinc-900 mb-1">Keperluan/Alasan</label>
                                    <textarea name="reason" rows="3" required
                                        class="block w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:ring-2 focus:ring-zinc-900 outline-none transition-all"
                                        placeholder="Tuliskan alasan/instruksi lembur..."></textarea>
                                </div>
                            </div>
                        </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div
                    ...
                    <form id="approvalForm" method="POST">
                        ...
                        @method('PATCH')
                        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <h3 class="text-lg font-bold text-zinc-900 mb-4">Approval Pengajuan Lembur</h3>
                            <div class="space-y-4">
                                <div>
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-lg font-bold text-zinc-900">Hapus Data Lembur</h3>
                                <p class="mt-2 text-sm text-zinc-500">Anda yakin ingin menghapus data lembur ini?</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function openApprovalModal(id) {
            document.getElementById('approvalForm').action = "{{ url('admin/overtime') }}/" + id + "/status";
            openModal('approvalModal');
        }

        function confirmDelete(id) {
            document.getElementById('deleteForm').action = "{{ url('admin/overtime') }}/" + id;
