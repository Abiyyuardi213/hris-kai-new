...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight">Edit Direktorat</h2>
        ...
        <div class="rounded-xl border bg-white shadow-sm p-6 max-w-2xl">
            <form action="{{ route('directorates.update', $directorate->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $directorate->name) }}"
                            required
                    ...
                    <div>
                        <label for="code" class="block text-sm font-medium text-zinc-900">Kode Direktorat
                            (Otomatis)</label>
                        <input type="text" name="code" id="code" value="{{ old('code', $directorate->code) }}"
                            required readonly
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Deskripsi singkat direktorat...">{{ old('description', $directorate->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
