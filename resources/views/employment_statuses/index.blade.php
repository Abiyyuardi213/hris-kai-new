...
@section('content')
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between pb-1">
                <p class="text-sm text-zinc-500">
                    Menampilkan <span class="font-medium text-zinc-900">{{ $statuses->firstItem() ?? 0 }}</span> -
                    <span class="font-medium text-zinc-900">{{ $statuses->lastItem() ?? 0 }}</span> dari
                    <span class="font-medium text-zinc-900">{{ $statuses->total() }}</span> status
                </p>
            </div>
                        ...
                        <tbody class="divide-y divide-zinc-100">
                            @forelse ($statuses as $status)
                                <tr class="group hover:bg-zinc-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <button onclick="copyToClipboard('{{ $status->id }}')"
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div
                                            ...
                                            <button onclick="showDetails({{ json_encode($status) }})"
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div
                    ...
                    <div class="bg-white px-6 py-4 border-b border-zinc-100 flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-zinc-900">Detail Status Pegawai</h3>
                    ...
                    <div class="bg-white px-6 py-6 space-y-6">
                        <div>
                            <label class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Kode Status</label>
                            <p id="detailCode"
                                class="mt-1 text-sm font-medium text-zinc-900 bg-zinc-50 px-3 py-2 rounded-lg border border-zinc-100">
                            </p>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Nama Status</label>
                            <p id="detailName" class="mt-1 text-base font-semibold text-zinc-900"></p>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Deskripsi</label>
    ...
    <script>
        ...
        function showDetails(status) {
            document.getElementById('detailCode').textContent = status.code;
            document.getElementById('detailName').textContent = status.name;
            document.getElementById('detailDescription').textContent = status.description || 'Tidak ada deskripsi.';
