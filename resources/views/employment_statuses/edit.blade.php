...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight">Edit Status Pegawai</h2>
        ...
        <div class="rounded-xl border bg-white shadow-sm p-6 max-w-2xl">
            <form action="{{ route('employment-statuses.update', $employmentStatus->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div class="space-y-4">
                    <div>
                        <label for="code" class="block text-sm font-medium text-zinc-900">Kode Status</label>
                        <input type="text" name="code" id="code"
                            value="{{ old('code', $employmentStatus->code) }}" required
                    ...
                    <div>
                        <label for="name" class="block text-sm font-medium text-zinc-900">Nama Status</label>
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="Contoh: Pegawai Kontrak">
                        @error('name')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Deskripsi status pegawai...">{{ old('description', $employmentStatus->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
