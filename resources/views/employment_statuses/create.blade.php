...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight">Tambah Status Pegawai</h2>
        ...
        <div class="rounded-xl border bg-white shadow-sm p-6 max-w-2xl">
            <form action="{{ route('employment-statuses.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required
                            ...
                            placeholder="Contoh: Tetap" oninput="generateCode()">
                        @error('name')
                    ...
                    <div>
                        <label for="code" class="block text-sm font-medium text-zinc-900">Kode Status (Otomatis)</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Deskripsi status pegawai...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
