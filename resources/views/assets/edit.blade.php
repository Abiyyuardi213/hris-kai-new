...
@section('content')
    <div class="flex flex-col space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight">Edit Aset</h2>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="purchase_date" id="purchase_date"
                                value="{{ old('purchase_date', $asset->purchase_date ? $asset->purchase_date : '') }}"
                        ...
                        <div>
                            ...
                            <select name="condition" id="condition" required
                                ...
                                <option value="repair"
                                    {{ old('condition', $asset->condition) == 'repair' ? 'selected' : '' }}>Perbaikan
                                </option>
                                <option value="broken"
                                    {{ old('condition', $asset->condition) == 'broken' ? 'selected' : '' }}>Rusak</option>
                                <option value="lost"
                                    {{ old('condition', $asset->condition) == 'lost' ? 'selected' : '' }}>Hilang</option>
                            </select>
                            @error('condition')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Deskripsi detail aset...">{{ old('description', $asset->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
